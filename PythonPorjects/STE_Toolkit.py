...
class VBS4Panel(tk.Frame):
    ...
    def create_mesh(self):
        try:
            ...
            messagebox.showerror("PhotoMesh", str(e), parent=self)
            return
        if not wait_for_queue_ready():
            messagebox.showerror(
                "PhotoMesh",
                "Project Queue API not available on localhost:8087",
                parent=self,
            )
            return
        ...
        try:
            self.log_message("Starting PhotoMesh so queue is available…")
            ensure_photomesh_queue_running(log=self.log_message, wait_seconds=90)
