...
class VBS4Panel(tk.Frame):
    ...
    def launch_reality_mesh_to_vbs4(self):
        ...
        local = ''
        if root:
            datatarget = is_valid_rm_local_root(root)
            if not datatarget:
                messagebox.showerror(
                    "Reality Mesh",
                    (
                        f"Reality Mesh local root '{root}' is invalid. Expected sentinel file 'Datatarget.txt' "
                ...
                return
            attempted = [
                os.path.normpath(os.path.join(root, sub, RM_LNK_NAME))
                for sub in RM_INSTALL_SUBDIRS
            ]
            local = find_local_rm_shortcut(root)
            if local:
                if dataset_root and get_drive_letter(root) != get_drive_letter(dataset_root):
                    tool_drive = get_drive_letter(root)
                    data_drive = get_drive_letter(dataset_root)
                    messagebox.showerror(
                        "Reality Mesh",
                        (
                            "Reality Mesh and data must be on the same drive. "
                            f"Current tool drive: {tool_drive}, data drive: {data_drive}."
                        ),
                    )
                    return
        ...
        if not os.path.isfile(link):
            ...
            install_dir = ''
            if root:
                install_dir = os.path.join(root, 'RealityMeshInstall')
            ...
            if root:
                msg_parts.append(
                    f"\nSentinel: {'found' if datatarget else 'missing'} at {os.path.normpath(root)}"
                )
            msg_parts.append(f"\nUNC path: {link}")
            if diag:
                msg_parts.append(f"\n{diag}")
