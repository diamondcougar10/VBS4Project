...
def find_reality_mesh_to_vbs4_link() -> str:
    ...
    default = resolve_unc(RM_TEMPLATE)
    if os.path.isfile(default):
        return default

    search_root = resolve_unc(RM_SEARCH_ROOT_TPL)
    return _find_reality_mesh_shortcut_under(search_root)
...
def _list_dir_safe(path: str, max_entries: int = 20) -> str:
    """Return a directory listing for *path* or a short error message."""
    try:
...
def _diagnose_missing_unc(path: str) -> str:
    """Return which segment of *path* is missing, skipping the host root."""
...
class VBS4Panel(tk.Frame):
    ...
    def launch_reality_mesh_to_vbs4(self):
        """Launch the Reality Mesh to VBS4 tool using a host-aware UNC path."""
